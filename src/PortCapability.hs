...
-- orig pcs type1s hash
data PortCapability = PortCapability String [PC] (HS.HashSet PC) Int

instance Eq PortCapability where
    PortCapability _ _ type1s _ == PortCapability _ _ type1s' _ = type1s == type1s'
instance Hashable PortCapability where
    hashWithSalt salt (PortCapability _ _ _ _hash) = salt * _hash

_hashType1s :: HS.HashSet PC -> Int
_hashType1s = foldr (hashWithSalt . hash) 17 . HS.toList

instance Show PortCapability where
    show (PortCapability orig _ _ _) = orig

instance Ord PortCapability where
    compare pc1@(PortCapability orig1 _ _ _) pc2@(PortCapability orig2 _ _ _)
...
portCap orig = trace orig $ 
               let pps = PP.parsePortCap orig
                   ...
                   _hash = _hashType1s type1s
               in PortCapability orig pcs type1s _hash

subPortCaps :: PortCapability -> HS.HashSet PortCapability
subPortCaps (PortCapability _ _ type1s _)
    = foldr (\pc subs -> HS.insert (portCap $ toPortCapString pc) subs) HS.empty $ HS.toList type1s

covers :: PortCapability -> PortCapability -> Bool
pc1 `covers` pc2 = HS.null $ HS.difference (subPortCaps pc2) (subPortCaps pc1)
...
pcForm (PortCapability _ [PC r rw w ru c a] _ _) -- length pcs == 1
    | lines' == 0        = PC_NOOP
    | lines' == r        = PC_nR
    | lines' == w        = PC_nW
    | lines' == rw       = PC_nRW
...
pcForm (PortCapability _ [PC r rw w ru c a, PC r' rw' w' ru' c' a'] _ _) -- length pcs == 2
