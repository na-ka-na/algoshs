...
gen2d :: [(Int,Int)]
gen2d = gen2d' 'D' (1,1)
    where gen2d' 'D' (x,1) = (x,1) : gen2d' 'R' (x+1,1)
          gen2d' 'R' (x,1) = (x,1) : gen2d' 'U' (x,2)
          gen2d' 'L' (1,y) = (1,y) : gen2d' 'U' (1,y+1)
          gen2d' 'U' (1,y) = (1,y) : gen2d' 'R' (2,y)
          gen2d' 'U' (x,y)
                  | x == y = (x,y) : gen2d' 'L' (x-1,x)
          gen2d' 'R' (x,y)
                  | x == y = (x,y) : gen2d' 'D' (x,y-1)
          gen2d' 'U' (x,y)
                  | y < x  = (x,y) : gen2d' 'U' (x,y+1)
          gen2d' 'L' (x,y)
                  | x < y  = (x,y) : gen2d' 'L' (x-1,y)
          gen2d' 'R' (x,y)
                  | x < y  = (x,y) : gen2d' 'R' (x+1,y)
          gen2d' 'D' (x,y)
                  | y < x  = (x,y) : gen2d' 'D' (x,y-1)
          gen2d' _ _ = error "illegal state"
...
memoize2 g = g'
    where idx2d (i,j) = fromJust $ elemIndex (i,j) gen2d
          cache = [g i j | (i, j) <- gen2d]
          g' i j = cache !! idx2d (i,j)

pc_nRW :: Int -> PortCapability
pc_nRW = memoize (\n -> portCap $ show n ++ "RW")
pc_nRor1W :: Int -> PortCapability
pc_nRor1W = memoize (\n -> portCap $ show n ++ "Ror1W")
pc_nRmW :: Int -> Int -> PortCapability
pc_nRmW = memoize2 (\n m -> portCap $ show n ++ "R" ++ show m ++ "W")
